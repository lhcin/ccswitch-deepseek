...
export class SseTranslator {
  constructor(res, resolvedModel) {
    this.res = res;
    this.responseId = "resp_" + crypto.randomUUID().slice(0, 10);
    this.messageItemId = "item_" + crypto.randomUUID().slice(0, 10);
  ...
  feed(chunk) {
    ...
    if (delta.reasoning_content) {
      ...
      if (!this.reasoningStarted) {
        ...
        this.reasoningItemId = "rsn_" + crypto.randomUUID().slice(0, 10);
        const oi = this.outputItemCount++;
